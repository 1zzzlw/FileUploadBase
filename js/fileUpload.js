import { sendFile } from './uploadApi.js';

const $ = document.querySelector.bind(document)

let fileList = []
let processList = []
let buttonAgainList = []
let buttonEndList = []

class fileInfoClass {
    constructor(file) {
        this.fileName = file.name
        this.fileType = file.type
        this.fileSize = file.size
        this.process = 0
        this.startProcess = 0
        this.endProcess = 100
        this.processList = []
    }

    getFileByIndex(index) {
        return fileList[index]
    }

    addprocessListDiv(processList) {
        processList.push(processList)
    }

}

const doms = {
    dragArea: $('.drag-area'),
    inputFile: $('.input-file'),
    inputDirectory: $('.input-directory'),
    uploadFile: $('.button-upload-file'),
    uploadDirection: $('.button-upload-directory'),
    downloadList: $('.download-list'),
    downloadProcessInfo: $('.download-processInfo'),
}

doms.uploadFile.onclick = () => {
    doms.inputFile.click()
}

doms.inputFile.onchange = (e) => {
    for (const file of e.target.files) {
        fileList.push(file)
    }
    updateInfoList(fileList)
}

doms.uploadDirection.onclick = () => {
    doms.inputDirectory.click()
}

doms.inputDirectory.onchange = (e) => {
    for (const file of e.target.files) {
        fileList.push(file)
    }
    updateInfoList(fileList)
}

// 进入
doms.dragArea.ondragenter = (e) => {
    // 禁用掉默认事件
    e.preventDefault();
    doms.dragArea.classList.add('dragenter')
}

// 停留
doms.dragArea.ondragover = (e) => {
    e.preventDefault();
    doms.dragArea.classList.add('dragenter')
}

// 离开
doms.dragArea.ondragleave = (e) => {
    e.preventDefault();
    doms.dragArea.classList.remove('dragenter')
}

// 松手
doms.dragArea.ondrop = (e) => {
    e.preventDefault();
    doms.dragArea.classList.remove('dragenter')
    // 获得拖拽的所有内容
    const items = e.dataTransfer.items
    for (const item of items) {
        const entry = item.webkitGetAsEntry()
        traverseDirectory(entry)
    }
}

function updateInfoList(fileList) {
    console.info(fileList)
    let html = ''
    const label = doms.downloadList.querySelector('.download-label')
    for (let i = 0; i < fileList.length; i++) {
        const file = fileList[i]
        const fileInfo = new fileInfoClass(file)
        const infoList = document.createElement('div')
        infoList.innerHTML = ` 
            <img src="./icon/wenjian.svg" alt="文件预览图片" class="file-preview" />
            <div class="download-processInfo">
                <div class="baseInfo">
                    <span>${fileInfo.fileName}</span>
                    <span>${fileInfo.fileType}</span>
                    <span>${fileInfo.fileSize}</span>
                    <span>状态</span>
                </div>  
                <div class="process process-${i}"></div>
            </div>
            <div class="button-control">
                <button class="button-again-${i} active">重新上传</button>
                <button class="button-end-${i}">取消上传</button>
            </div>
        `
        // html += `   
        // <div class="info-list">
        //     <img src="./icon/wenjian.svg" alt="文件预览图片" class="file-preview" />
        //         <div class="download-processInfo">
        //             <div class="baseInfo">
        //                 <span>${fileInfo.fileName}</span>
        //                 <span>${fileInfo.fileType}</span>
        //                 <span>${fileInfo.fileSize}</span>
        //                 <span>状态</span>
        //             </div>  
        //             <div class="process process-${i}"></div>
        //         </div>
        //         <div class="button-control">
        //             <button class="button-again-${i} active">重新上传</button>
        //             <button class="button-end-${i}">取消上传</button>
        //         </div>
        // </div>     
        // `

        /** 
         * 传递一个就上传一个，关键是需要记住process的进度
         * 否则选择新的文件长传的时候会刷新进度为0
         */
        // uploadFile(fileInfo, i)
        doms.downloadList.appendChild(infoList)
    }
    // 重新添加表头元素，并覆盖原来的值重新进行赋值
    // doms.downloadList.replaceChildren(label)
    // doms.downloadList.innerHTML += html

}

// 遍历文件夹
function traverseDirectory(entry) {
    if (entry.isDirectory) {
        // 如果是文件夹，重新遍历 先创建一个文件夹读取器
        const reader = entry.createReader()
        reader.readEntries((entrys) => {
            for (const entry of entrys) {
                if (entry.isDirectory) {
                    // 如果内部还是文件夹，重新遍历
                    traverseDirectory(entry)
                } else {
                    // 是文件，直接加入文件列表
                    entry.file((file) => {
                        fileList.push(file)
                        updateInfoList(fileList)
                    })
                }
            }
        })

    } else {
        // 如果是文件，直接更新文件列表
        console.info(entry)
        entry.file((file) => {
            fileList.push(file)
            updateInfoList(fileList)
        })
    }
}

async function uploadFile(fileInfo, index) {
    const formData = new FormData()

    const file = fileInfo.getFileByIndex(index)

    formData.append('file', file)

    // 配置监听传输的进程
    const config = {
        onUploadProgress: (e) => {
            // 如果文件大小未知，直接退出
            if (!e.lengthComputable) return
            const loaded = e.loaded
            // 四舍五入
            const process = Math.round(loaded / file.size * 100)
            processList[index].style.setProperty('--process', process)
        }
    }

    await sendFile(formData, config).then((res) => {
        console.info('发送成功', res.data)
    }).catch((error) => {
        console.info(error)
    })
}

doms.downloadList.addEventListener('click', (e) => {
    const child = doms.downloadList.querySelectorAll('.info-list')
    for (let i = 0; i < child.length; i++) {
        processList.push(child[i].querySelector(`.process-${i}`))
        buttonAgainList.push(child[i].querySelector(`.button-again-${i}`))
        buttonEndList.push(child[i].querySelector(`.button-end-${i}`))
        if (e.target.classList.contains(`button-again-${i}`)) {
            processList[i].classList.remove('active')
            buttonEndList[i].classList.remove('active')
            buttonAgainList[i].classList.add('active')
        } else if (e.target.classList.contains(`button-end-${i}`)) {
            processList[i].classList.add('active')
            buttonEndList[i].classList.add('active')
            buttonAgainList[i].classList.remove('active')
        }
    }
    console.info(processList)
})

window.setProcess = function (index) {
    console.info(processList[index])
    processList[index].style.setProperty('--process', 50)
}

